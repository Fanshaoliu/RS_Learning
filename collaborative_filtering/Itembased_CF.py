...
for i in train_index:
    train_data_[A[i][0]-1][A[i][1]-1] = A[i][2]

for i in test_index:
    test_data_[A[i][0]-1][A[i][1]-1] = A[i][2]

train_data_ = np.transpose(train_data_)
test_data_ = np.transpose(test_data_)
...
# 计算预测矩阵
# def pred(sim, rating, mean, N=None):
#     ind = np.argsort(sim)[:, -N:]
#     Sim = np.zeros_like(sim)
#     for i in range(len(ind)):
#         Sim[i][ind[i]] += sim[i][ind[i]]
#     sim = Sim
#
#     Ra = np.tile(mean, (1, np.shape(mean)[-1]))
#     t = np.sum(sim, axis=1, keepdims=True)
#     sim_ab = sim / (np.tile(t, (1, np.shape(t)[-1])) + 1e-6)
#     Rbp_Rb = rating - np.tile(mean, (1, np.shape(mean)[-1]))
#     pred_rating = Ra + np.matmul(sim_ab, Rbp_Rb)
#     return pred_rating
